...
<template>
  <header class="flex-between">
    ...
    <div class="flex items-center space-x-2 m-2 cursor-pointer" @click="router.push('/')">
      ...
      <h1 class="text-2xl md:text-4xl lg:text-5xl italic font-bold py-1 md:py-3 lg:py-5 text-gray-600">
        Food <span class="text-orange-600">Partner</span>
      </h1>
    </div>

    <div class="block sm:hidden flex items-center space-x-3 mr-5">
      <div v-if="authStore?.userData?.userId" class="relative cursor-pointer hover:bg-orange-50 rounded-full"
    ...
    <nav class="navigation-menu">
      ...
      <RouterLink v-if="authStore?.userData?.userId" to="/restaurant" class="router-link">Restaurant</RouterLink>
      <RouterLink v-if="authStore?.userData?.userId && authStore?.userData?.role == 'owner'" to="/combo"
        class="router-link">Combo</RouterLink>
      <RouterLink v-if="authStore?.userData?.userId && authStore?.userData?.role == 'owner'" to="/item"
        class="router-link">Item</RouterLink>
      <RouterLink v-if="authStore?.userData?.userId && authStore?.userData?.role == 'owner'" to="/category"
        class="router-link">Category</RouterLink>
